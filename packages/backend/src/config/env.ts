...
const envSchema = z.object({
  ...
  DATABASE_URL: emptyStringToUndefined(z.string().url().optional()),
  PUBLIC_BASE_URL: emptyStringToUndefined(z.string().url().optional()),
  ...
  OASIS_RPC_URL: emptyStringToUndefined(z.string().url().optional()),
  OASIS_RPC_FALLBACK_URLS: emptyStringToUndefined(z.string().optional()),
  OASIS_EXPECTED_CHAIN_ID: z.coerce.number().int().positive().default(0x5aff),
  BACKEND_SIGNER_PRIVATE_KEY: emptyStringToUndefined(
    z.string().regex(evmPrivateKeyRegex, "Invalid EVM private key").optional(),
  ),
  CONTRACT_ADDRESS: emptyStringToUndefined(
    z.string().regex(evmAddressRegex, "Invalid contract address").optional(),
  ),
  BLOCKCHAIN_TX_TIMEOUT_MS: z.coerce.number().int().min(5000).max(180000).default(45000),
  HISTORY_SYNC_API_KEY: emptyStringToUndefined(z.string().min(24).optional()),
